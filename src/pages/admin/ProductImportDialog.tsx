...
export function ProductImportDialog({
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      setResult(outcome)
      onImported()
      toast.success(`${outcome.created} produit${outcome.created > 1 ? 's' : ''} importé${outcome.created > 1 ? 's' : ''}.`)
    } catch {
  ...
  return (
    <Dialog open={open} onClose={handleClose} title="Importer des produits (CSV)" size="lg">
      {result ? (
