...
const shop = (over: Partial<PlatformShop>): PlatformShop => ({
  ...
  country_code: 'SN', business_type: 'mode', subscribed_plan: 'free', period_end: null, last_order_at: day(-5), suspended_at: null, ...over,
})
...
const shops = [
  ...
  shop({ id: 'suspended', name: 'Suspendue', suspended_at: day(-1) }),
...
describe('filterShops', () => {
  it('cherche sans accent ni casse dans le nom, le slug, l’email et le WhatsApp', () => {
    ...
    expect(filterShops(shops, { ...DEFAULT_SHOP_FILTERS, query: '771234567' }, NOW)).toHaveLength(6)
  })
  ...
  it('filtre par plan effectif, échéance proche et échu récent', () => {
    ...
    expect(ids(filterShops(shops, { ...DEFAULT_SHOP_FILTERS, plan: 'free' }, NOW))).toEqual(['free', 'lapsed', 'suspended', 'old-lapsed'])
  ...
  it('isole les boutiques suspendues', () => {
    expect(ids(filterShops(shops, { ...DEFAULT_SHOP_FILTERS, plan: 'suspended' }, NOW))).toEqual(['suspended'])
  })
  ...
  it('filtre par pays et par activité', () => {
    ...
    expect(ids(filterShops(shops, { ...DEFAULT_SHOP_FILTERS, activity: 'active' }, NOW))).toEqual(['pro-soon', 'lapsed', 'suspended', 'old-lapsed'])
  })
})
...
describe('summarizeSubscriptions', () => {
  it('calcule le MRR des seuls abonnements en cours et repère ceux à relancer', () => {
    ...
    expect(summary.byPlan).toEqual({ free: 4, essential: 1, pro: 1 })
    expect(summary.payingShops).toBe(2)
    expect(summary.paidShare).toBe(33)
