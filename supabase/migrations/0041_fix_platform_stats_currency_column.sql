-- Bug fix: get_platform_stats() (0033, restated in 0035) grouped revenue by
-- `currency` selected directly from public.orders — but orders has no
-- currency column, only shops does (see 0001_init.sql). Every call raised
-- "column \"currency\" does not exist", which SuperAdminPage.tsx treats as a
-- hard failure and renders in place of the entire super-admin tool (all five
-- tabs — Vue d'ensemble, Analytiques, Boutiques, Commandes, Paiements — never
-- got past this one query). get_platform_shops() already joins to
-- public.shops for this same value; this brings get_platform_stats() in
-- line with it.
