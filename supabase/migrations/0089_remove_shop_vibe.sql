-- Les ambiances ont été retirées du produit (la personnalisation passe par les
-- styles et le thème dans « Personnaliser »). On supprime la colonne
-- `shops.vibe` et on réécrit `platform_audience` sans la filtre `vibe`, pour
-- que le split Apparence / Mise en page et les campagnes restent cohérents.
--
-- Les anciens `campaigns.audience` (jsonb) contenant une clé `vibe` sont
-- inoffensifs : la fonction ne la lit plus.

alter table public.shops drop column if exists vibe;
