-- Product variants: optional per-variant stock/price/SKU on a product. A
-- variant is a free-form name ("Taille M", "Rouge — 42", ...) — no generated
-- option grid. When a product has variants, order/stock math uses the variant
-- rows; products.stock/products.price remain the base defaults.
...
-- RLS calquée sur product_images (0002_rls.sql) : lisible quand le produit
-- parent l'est, écrite uniquement par le propriétaire de la boutique.
create policy "product_variants: read with product" on public.product_variants
  for select using (
    exists (
...
-- order_items: snapshot au moment de la commande (nom de variante), survit à
-- la suppression de la variante (set null) comme product_id.
...
-- create_order(): mêmes arguments qu'avant — seul le format de p_items gagne
-- un `variant_id` optionnel par ligne. Toujours en deux passes (verrouille et
-- valide tout, puis écrit).
...
create or replace function public.create_order(
  ...
  p_items jsonb, -- [{ "product_id": uuid, "quantity": int, "variant_id": uuid? }, ...]
